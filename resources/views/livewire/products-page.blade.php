            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach ($categories as $category)
                                <li class="mb-4" wire:key="category-{{ $category->id }}">
                                    <label for="category-{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" wire:model.live="selected_categories" id="category-{{ $category->slug }}" value="{{ $category->id }}"
                                            class="w-4 h-4 mr-2">
                            ...
                            @foreach ($brands as $brand)
                                <li class="mb-4" wire:key="brand-{{ $brand->id }}">
                                    <label for="brand-{{ $brand->slug }}" class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" wire:model.live="selected_brands" id="brand-{{ $brand->slug }}" class="w-4 h-4 mr-2"
                                            value="{{ $brand->id }}">
                        ...
                        <ul>
                            <li class="mb-4">
                                <label for="is_featured" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="is_featured" value="1" wire:model.live="selected_status_is_featured" class="w-4 h-4 mr-2">
                                    <span class="text-lg dark:text-gray-400">Featured Products</span>
                                </label>
                            </li>
                            <li class="mb-4">
                                <label for="on_sale" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" wire:model.live="selected_status_on_sale" id="on_sale" value="1" class="w-4 h-4 mr-2">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold">{{ Number::currency($price_range, "COP") }}</div>
                            <input type="range" wire:model.live="price_range"
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="1000000" value="5000" step="5000">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(5000, "COP") }}</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(1000000, "COP") }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="px-3 mb-4">
                        <div
                            ...
                            <div class="flex items-center justify-between">
                                <div class="block w-40 text-base bg-gray-100 cursor-pointer dark:text-gray-400 dark:bg-gray-900">
                            ...
                            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $product->id }}">
                                <div class="border border-gray-300 dark:border-gray-700">
                                    ...
                                    <div class="p-3 ">
                                        ...
                                        <p class="text-lg ">
                                            <span
                                                class="text-green-600 dark:text-green-600">${{ number_format($product->sale_price ?? $product->price, 0, ',', '.') }}</span>
                                            @if ($product->sale_price && $product->sale_price < $product->price)
                                                <span
                                                    class="text-gray-500 line-through ml-2">${{ number_format($product->price, 0, ',', '.') }}</span>
                                            @endif
                                        </p>
                                    </div>
